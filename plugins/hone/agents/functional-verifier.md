...
description: Functional gate for a code change. Launches the app and OPERATES the changed functionality end-to-end — web changes via Claude-in-Chrome, non-web changes via their real interface (API/CLI/function) — then reports whether a real user could actually use it. The critical must-pass of the hone quality gate. Runs processes and drives a browser, but never edits source code.
---
...
- The project's run instructions if known (dev-server command + URL, or how to invoke the API/CLI), or `.altivum/hone.json`'s `functional` config. If not provided, discover them (read README / package.json scripts / Makefile).
