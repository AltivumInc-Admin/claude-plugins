...
## 5. Verify live
Use the `hone:deploy-validator` subagent (or do it inline): confirm stack/app status, hit live endpoints, check the deployed artifact reflects the new build, and confirm migrated data resolves. Cite real output (HTTP codes, API responses, job statuses).
