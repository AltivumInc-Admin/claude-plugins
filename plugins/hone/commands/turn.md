...
4. **DEPLOY** — per `/hone:deploy`: feasibility first, reviewable/reversible mechanisms, dry-run migrations, confirm at the point of no return, verify live. Finish with a debrief AND append a journal entry to `.altivum/journal.md` (and the vault mirror per `~/.altivum/hone.json`), using the same journal/vault mechanics as `/hone:loop`'s RECORD step — create the `# Hone Journal` heading if absent, apply the collision guard, and skip an unavailable vault with a one-line note rather than failing the turn:
