---
description: Generate detailed, actionable implementation plan(s) for selected gaps or a stated goal.
argument-hint: "[gap number(s) like '1 3' or 'all', or a goal]"
---

# Implementation Plan Generator

Generate detailed, executable implementation plan(s) based on **$ARGUMENTS** (numbered gaps from a gap report, or a stated goal).

## Parse the request
- A single number -> plan that one. Multiple numbers -> plan each. "all" -> plan all gaps from the prior report. A goal -> plan that goal.
- If nothing is provided, ask which gap(s) to plan.

## For EACH selected item, produce all of these sections (do not skip or abbreviate)

**Objective** — 2-3 sentences: the goal, what changes when done, and (when `.altivum/intent.md` exists) the intent clause this serves — quote it from the gap being planned, or note "none cited".
