#!/usr/bin/env bash
# PreToolUse(Bash) BLOCKING gate for hone.
#
...
#     HONE_DEPLOY_APPROVED=1 <command>
#
...
if printf '%s' "$input" | grep -qiE "$DEPLOY_RE"; then
  ...
  printf '%s\n' \
    "BLOCKED by hone pre-deploy gate." \
    ...
    "Then re-run the SAME command prefixed with:  HONE_DEPLOY_APPROVED=1 <command>" >&2
  exit 2  # block
fi
