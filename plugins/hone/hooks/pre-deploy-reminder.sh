...
if printf '%s' "$input" | grep -qiE 'sam[[:space:]]+deploy|execute-change-set|cloudformation[[:space:]]+(deploy|create-change-set|update-stack)|amplify[[:space:]]+(start-job|update-app)|cdk[[:space:]]+deploy|terraform[[:space:]]+apply|serverless[[:space:]]+deploy'; then
  printf '%s\n' \
    "[hone] Production-deploy command detected. Pre-flight:" \
